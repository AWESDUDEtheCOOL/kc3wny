        ...
        if (result.skipped) {
          ...
          if (result.optimized) {
            ...
            if (result.formatChange) {
              console.log(`   ✓ ${result.oldPath} → ${path.basename(result.newPath)} (saved ${(result.saved/1024/1024).toFixed(1)}MB)`)
            } else {
              console.log(`   ✓ ${relativePath} (saved ${(result.saved/1024/1024).toFixed(1)}MB)`)
            }
          } else {
